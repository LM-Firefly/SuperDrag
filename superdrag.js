  ...
  link_draggable = function () {
    for (let link of document.links) {
      ...
      if (link.childElementCount > 0) {
        ...
        if (
          Math.abs(rect.height - link.offsetHeight) > 1 ||
          Math.abs(rect.width - link.offsetWidth) > 1
        ) {
...
class SuperDrag {
  constructor(superDrag) {
    ...
    document.addEventListener(
      "dragstart",
      (ev) => this.dragstart(ev, superDrag),
      false
    );
    document.addEventListener(
      "dragover",
      (ev) => this.dragover(ev, superDrag),
      false
    );
    document.addEventListener(
      "dragend",
      (ev) => this.dragend(ev, superDrag),
      false
    );
  }
  ...
  dragover(event, superDrag) {
    if (!this.notice) {
      ...
      this.notice.style.cssText =
        "font-family:siyuan;max-width:60%;min-width: 150px;padding:0 14px;height: 40px;color: rgb(255, 255, 255);line-height: 40px;text-align: center;border-radius: 4px;position: fixed;top: 90%;left: 50%;transform: translate(-50%, -50%);z-index: 999999;background: rgba(0, 0, 0,.7);font-size: 16px;";
      if (
        superDrag.superDrag.showNotice &&
        (["A", "IMG", "#text"].indexOf(this.dragEvent.srcElement.nodeName) !=
          -1 ||
    ...
    if (
      superDrag.superDrag.timeout !== 0 &&
      superDrag.superDrag.timeout < time_collect
    ) {
      this._dic.timeout = true;
    }
    if (
      superDrag.superDrag.enableAlt &&
      (event.altKey || (this.isMac && event.metaKey))
    ) {
      this.toCancel = true;
    }
    if (
        ...
        if (this.dragEvent.srcElement.localName == "a") {
          ...
          if (this.containsImg) {
            ...
            if (superDrag.superDrag.firstEvent) {
              if (superDrag.superDrag.link_type[position_link] === 3) {
                ...
                this.notice.innerHTML =
                  this.arrow[superDrag.superDrag.direction_sel][position_link] +
                  " 链接 - " +
                  _link_type[superDrag.superDrag.link_type[position_link]];
              }
              if (superDrag.superDrag.link_type[position_link] === 0) {
          ...
          if (urlPattern.test(keyword)) {
            ...
            if (superDrag.superDrag.text_type[position_text] === 0) {
              if (superDrag.superDrag.searchEngines[position_text].url) {
                this.notice.innerHTML =
                  this.arrow[superDrag.superDrag.direction_sel][position_text] +
                  " 文本 - " +
                  _text_type[superDrag.superDrag.text_type[position_text]] +
                  " - " +
                  chrome.i18n.getMessage("custom_search");
  ...
  dragend(event, superDrag) {
    if (
      !this._dic.timeout &&
      !this.toCancel &&
      ["link", "none"].indexOf(this.dragEvent.dataTransfer.dropEffect) != -1
    ) {
    ...
    if (
      document.getElementById("notice-superDrag" + this._dic.start_time) != null
    ) {
  ...
  qrcode(something) {
    ...
    n.style.cssText =
      ...
      "px;height: " +
      height_n +
      "px;z-index: 2147483646;filter: opacity(0.8);background-color: slategray;";
...
class OpenLinks {
    ...
    if (this.keyCode) {
      if (
        ...
        this.keyCode.toUpperCase() ==
          this.setting.superDrag.keyCode.toUpperCase()
      ) {
        ...
        document.addEventListener(
          "mousemove",
          (ev) => this.mousemove(ev, event),
          false
        );
        document.addEventListener(
          "mouseup",
          (ev) => this.mouseup(ev, event),
          false
        );
      }
    }
  ...
  links_handle() {
    ...
    for (var i = 0; i < this.page_links.length; i++) {
      ...
      // reject href="" or href="#"
      if (
        !this.page_links[i].getAttribute("href") ||
        this.page_links[i].getAttribute("href") === "#"
      ) {
        continue;
      }
      ...
      for (var k = 0; k < this.page_links[i].childNodes.length; k++) {
        if (this.page_links[i].childNodes[k].nodeName == "IMG") {
          ...
          if (pos.y >= pos2.y) {
            ...
            width = Math.max(
              width,
              this.page_links[i].childNodes[k].offsetWidth
            );
            height = Math.max(
              height,
              this.page_links[i].childNodes[k].offsetHeight
            );
          }
        }
      }
      ...
      this.page_links[i].important =
        this.page_links[i].parentNode != null &&
        re2.test(this.page_links[i].parentNode.nodeName);
  ...
  highlight_link() {
    ...
    for (var i = 0; i < this.links.length; i++) {
      if (
        (!this.smart_select || this.links[i].important) &&
        !(
          this.links[i].x1 > this.box.x2 ||
          this.links[i].x2 < this.box.x1 ||
          this.links[i].y1 > this.box.y2 ||
          this.links[i].y2 < this.box.y1
        )
      ) {
